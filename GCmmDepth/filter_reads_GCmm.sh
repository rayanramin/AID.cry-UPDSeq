...
samtools view -h $input | grep -v "NM:i:0" | grep -E "MD:Z:.*([0-9]{1,3}[CG][0-9]{1,3})" | cat Xheader.txt - |  samtools view -S 
rm Xheader.txt
fi
...
samtools view -H $input > Xheader.txt
samtools view -h $input | grep -v "NM:i:0" | grep -E "MD:Z:.*([0-9]{1,3}[CG][0-9]{1,3})" | cat Xheader.txt - |  samtools view -Sb  > $output
rm Xheader.txt
